    ...
    (set, get) => ({
      ...
      login: async (email: string, password?: string) => {
        ...
        try {
          ...
          if (
            (res.status === 0 || res.status === 1000 || res.data?.token) &&
            res.data &&
            res.data.account
          ) {
            ...
            if (role !== "STUDENT" && role !== "LECTURER") {
              throw new Error(
                "Tài khoản của bạn không có quyền đăng nhập vào ứng dụng này!",
              );
            }
          ...
          if (res.status === 1000 && res.data) {
            ...
            if (userInfo?.picture && accountId) {
              try {
                ...
                console.warn(
                  "[Google Auth] Failed to update avatar:",
                  avatarError,
                );
              }
            }
